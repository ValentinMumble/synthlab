...
/**
 * The VCFLP controller
 * 
 * @author valentinmumble
 * 
 */
public class CVoltageControlledFilterLowPass extends
        AVoltageControlledFilterLowPass implements
        ICVoltageControlledFilterLowPass {

    private IPVoltageControlledFilterLowPass presentation;

    private ICFemaleJack input;
    private ICFemaleJack freqModInput;
    private ICFemaleJack output;

    public CVoltageControlledFilterLowPass(Synthesizer syn) {
        super(syn);
        ...
        presentation = new PVoltageControlledFilterLowPass(this);
    }
    ...
    @Override
    public void resonanceChanged(double res) {
        setResonance(res);
        presentation.updateResonance(getResonance());
    }
    ...
    public List<SaveAttenuation> getAttenuationForSave() {
        ...
        SaveAttenuation att2 = new SaveAttenuation("reso", this.getResonance());
        listAtt.add(att1);
        listAtt.add(att2);
        return listAtt;
    }
        ...
        for (int i = 0; i < listAttenuationToInitialize.size(); i++) {
            if (listAttenuationToInitialize.get(i).getIdent().equals("freq")) {
                this.cutoffFrequencyChanged(listAttenuationToInitialize.get(i)
                    ...
                    .equals("reso")) {
                this.resonanceChanged(listAttenuationToInitialize.get(i).getValue());
            }
        }
    }
